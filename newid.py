...
# -*- coding: utf-8 -*-
"""
wxExcelDiffTool.py — 可配置的多 Sheet Excel 对比工具（wxPython + pandas）

功能概述：
1) 选择表A与表B（支持 .xlsx/.xls），自动读取 Sheet 列表
2) 勾选需要对比的 Sheet
3) 为每个 Sheet 配置：ID 列、表A对比列、表B对比列（可不同）
4) 选项：忽略大小写、去除前后空格
5) 一键比对，GUI 表格展示结果，支持导出 Excel 差异报告（含汇总页 + 每个 Sheet 的结果页，颜色高亮）

依赖：
    pip install wxPython pandas openpyxl xlsxwriter
在 Windows 安装 wxPython 可参考：https://wxpython.org/pages/downloads/
